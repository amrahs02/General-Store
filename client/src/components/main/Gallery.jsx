...
const Gallery = () => {
  ...
  return (
    <div className="gallery-container text-center rounded-2xl bg-gray-700 ">
      <h2 className="text-center text-3xl text-white font-bold mt-6 p-4 rounded-2xl  bg-slate-800 ">
        {shopHeading}
      </h2>
      <div className="w-fit mt-2 sm:px-44  grid grid-cols-1 rounded-2xl sm:grid-cols-2  md:grid-cols-3 lg:grid-cols-4 gap-4">
        {allImages.slice(0, visibleImages).map((image, index) => (
          <img
            key={index}
            src={image}
            alt={`Shop Image ${index + 1}`}
            className="w-full rounded-2xl h-64 object-cover bg-slate-800 shadow-md transition-transform transform hover:scale-105 animate__animated animate__fadeIn"
          />
        ))}
      </div>
      {visibleImages !== allImages.length && (
        <button
          ...
          className=" m-4 px-4 py-2  text-white bg-blue-500 rounded-2xl hover:bg-blue-600 focus:outline-none focus:shadow-outline-blue active:bg-blue-800 "
        >
          See More
        </button>
      )}
